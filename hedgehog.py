...
class Hedgehog():
    ...
    def printActivity(self,a,printimage=False):
        ...
        if inboth("begindate",act,actstruct):
            if inboth("enddate",act,actstruct):
                actstring+=self.printDates(act['begindate']['$date'],act['enddate']['$date'])
            else:
                actstring+=act['begindate']['$date'].split("T")[0]
            actstring+="\\\\ \n" 

        if inboth("location",act,actstruct):
            actstring+="{\\em "+actstruct["location"]["viewValue"]+" :} "+act["location"]
            actstring+="\\\\ \n" 

        if inboth("comment",act,actstruct):
            if len(act["comment"])>5:
                actstring+="{\\em "+actstruct["comment"]["viewValue"]+" :} "+act["comment"]
                actstring+="\\\\ \n"  

        return actstring


    def printCourse(self,c):
        course=self.courses[c]
        _id=course["_id"]["$oid"]

        corstring=""

        corstring+="{\\bf "+course["title"]+" } \\\\ \n"

        if findAndLenGtZero("maininternalteachers",course) or findAndLenGtZero("mainexternalteachers",course):
            corstring+="{\\em Main teacher: } "
            if findAndLenGtZero("maininternalteachers",course):                
                for internal in course["maininternalteachers"]:                       
                    corstring+= "{\\bf "+self.printPerson(internal)+"}, "
            if findAndLenGtZero("mainexternalteachers",course):
                for external in course["mainexternalteachers"]:                       
                    corstring+= external+", "
            corstring+=" \\\\\n"

        if findAndLenGtZero("internalteachers",course) or findAndLenGtZero("externalteachers",course):
            corstring+="{\\em Teachers:} "
            if findAndLenGtZero("internalteachers",course):                
                for internal in course["internalteachers"]:                       
                    corstring+= "{\\bf "+self.printPerson(internal)+"}, "
            if findAndLenGtZero("externalteachers",course):
                for external in course["externalteachers"]:                       
                    corstring+= external+", "
            corstring+=" \\\\\n"

        if findAndLenGtZero("internalexaminer",course) or findAndLenGtZero("externalexaminer",course):
            if findAndLenGtZero("internalexaminer",course) in course:
                corstring+="{\\em Examiner:} "+self.printPerson(course["internalexaminer"])
            elif findAndLenGtZero("externalexaminer",course) in course:
                corstring+="{\\em Examiner:} "+course["externalexaminer"]
            corstring+=" \\\\\n"

        corstring+="{\\em Period:} "
        if "enddate" in course:
            corstring+=self.printDates(course['begindate']['$date'],course['enddate']['$date'])
        else:
            corstring+=course['begindate']['$date'].split("T")[0]+" - Current"
        corstring+=" \\\\\n" 

        if findAndLenGtZero("description",course):
            corstring+="{\\em Description:} "+course['description']
